...
class Prefs(context: Context) {
    ...
    /** Se true, il visualizer viene renderizzato a risoluzione ridotta per
     * andare più leggero su dispositivi lenti (es. head unit economiche). */
    var performanceMode: Boolean
        get() = sp.getBoolean("performance_mode", false)
        set(value) = sp.edit().putBoolean("performance_mode", value).apply()

    /** True dopo che l'app ha già provato a indovinare se il dispositivo è
     * "debole" e impostato performanceMode di conseguenza al primo avvio:
     * non lo rifà più dopo, per non sovrascrivere una scelta manuale. */
